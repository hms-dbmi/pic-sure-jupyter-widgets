...
var LoginWindowModel = widgets.DOMWidgetModel.extend({
    defaults: _.extend(widgets.DOMWidgetModel.prototype.defaults(), {
        _model_name : 'LoginWindowModel',
        _view_name : 'LoginWindowView',
        _model_module : 'pic-sure-jupyter-widgets',
        _view_module : 'pic-sure-jupyter-widgets',
        _model_module_version : '1.1.1',
        _view_module_version : '1.1.1',
...
// Custom View. Renders the widget model.
var LoginWindowView = widgets.DOMWidgetView.extend({
    // Defines how the widget gets rendered into the DOM
    render: function() {
        var wrapper_el = document.createElement("div");
        var target_el = document.createElement("div");
        target_el.style.float = "left";
        target_el.style.width = "500px";
        target_el.style.height = "70px";
        target_el.style.padding = "12px 10px 0px 80px";
        target_el.style.border = "1px solid black";
        ...
        button.style = "float: left; height:76px; margin-right: 12px";
        var closure_url = this.model.get("windowURL");
        var clickhandler = function() {
            window.open(closure_url,'_blank','height=1060,width=850,menubar=no');
        }
        button.addEventListener("click", clickhandler);
        button.innerHTML = "Show Login";

        wrapper_el.appendChild(button);
        wrapper_el.appendChild(target_el);
        this.el.appendChild(wrapper_el);
    }
});
...
module.exports = {
    ...
    LoginWindowView: LoginWindowView
};
